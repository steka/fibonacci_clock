...
//    APP_LOG(APP_LOG_LEVEL_INFO, "Load config dotColor = %d", conf.dotColor.argb);
  ...
  if (persist_exists(ConfTimeColor)) {
    conf.timeColor.argb = persist_read_int(ConfTimeColor);
...
//    }
    ...
    switch(t->key) {
      case ConfShowTime:
        conf.showTime = atoi(t->value->cstring);
      ...
      case ConfDotColor:
        ...
        break;
      case ConfTimeColor:
        conf.timeColor = (GColor){.argb = atoi(t->value->cstring)};
        persist_write_int(t->key, conf.timeColor.argb);
        update_time();
        break;
